...
![](../assets/0033-基于AVScreenCapture实现屏幕录制-bpta-avscreencapture-for-screen-recording-image-001.gif "点击放大")
...
![](../assets/0033-基于AVScreenCapture实现屏幕录制-bpta-avscreencapture-for-screen-recording-image-002.png)
...
1. 获取文件信息。     首先，通过时间戳和应用沙箱目录拼接出文件路径，然后利用文件管理模块的openSync()接口获取文件信息。后续的录屏文件将存储在该文件中。
...
![](../assets/0033-基于AVScreenCapture实现屏幕录制-bpta-avscreencapture-for-screen-recording-image-003.gif "点击放大")
...
![](../assets/0033-基于AVScreenCapture实现屏幕录制-bpta-avscreencapture-for-screen-recording-image-004.png "点击放大")
...
2. 创建AVScreenCapture实例化对象并初始化。     首先，获取了ArkTS侧的参数信息，这些参数在配置录屏参数时将被使用。接着，调用OH_AVScreenCapture_Create()创建实例对象，随后配置录制参数并初始化AVScreenCapture对象。
...
3. 绑定回调函数     这里设置了 stateChange（状态切换事件回调）和 error（错误事件回调）。
...
![](../assets/0033-基于AVScreenCapture实现屏幕录制-bpta-avscreencapture-for-screen-recording-image-005.gif "点击放大")
...
![](../assets/0033-基于AVScreenCapture实现屏幕录制-bpta-avscreencapture-for-screen-recording-image-006.png "点击放大")
...
2. 初始化音频编码器、视频编码器以及封装器。     首先，也需要获取ArkTS侧的参数，这部分内容在前面已经介绍过，这里不再赘述。具体可参考使用AVScreenCapture模块进行录屏存文件的章节。
