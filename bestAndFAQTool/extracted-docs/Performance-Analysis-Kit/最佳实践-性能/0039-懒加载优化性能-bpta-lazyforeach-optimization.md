...
- 循环渲染     ForEach循环渲染的过程如下：
   ...
   **图1** ForEach渲染过程示意图   ![](../assets/0039-懒加载优化性能-bpta-lazyforeach-optimization-image-001.png "点击放大")
...
- 数据懒加载     LazyForEach懒加载的原理和渲染过程如下：
   ...
   **图2** LazyForEach渲染过程示意图
     ![](../assets/0039-懒加载优化性能-bpta-lazyforeach-optimization-image-002.png "点击放大")
...
在LazyForEach的使用上，有一些限制条件和限制场景，详细的限制条件请参考[使用限制](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-rendering-control-lazyforeach#%E4%BD%BF%E7%94%A8%E9%99%90%E5%88%B6)，常见的限制场景总结如下：
...
2. 通过Profiler调优工具抓取Trace，可以判断子组件创建的次数。如下图所示，在该帧中出现大量的BuildLazyItem切片，每一次BuildLazyItem对应一次子组件的创建，对比数量可知LazyForEach按需加载失效。关于调优的内容可参考[性能分析](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-optimization-tool-practice)。     ![](../assets/0039-懒加载优化性能-bpta-lazyforeach-optimization-image-003.png "点击放大")
...
在LazyForEach的键值生成规则中，每个item对应着一个唯一且持久的键值，用于标识对应的组件。当不同的数据项有相同的键值时，框架可能找不到正确的数据项，导致子组件渲染错误。关于键值错误导致渲染错乱的案例，详情可以参考[首次渲染](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-rendering-control-lazyforeach#%E9%A6%96%E6%AC%A1%E6%B8%B2%E6%9F%93)中的示例。
