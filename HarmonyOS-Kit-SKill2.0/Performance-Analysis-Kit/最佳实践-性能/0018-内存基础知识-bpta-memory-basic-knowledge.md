...
![](../assets/0018-内存基础知识-bpta-memory-basic-knowledge-image-001.png "点击放大")
...
![](../assets/0018-内存基础知识-bpta-memory-basic-knowledge-image-002.png "点击放大")
...
| 名称 | 用途 | 分配时机 | 特点 |
| --- | --- | --- | --- |
| 栈（Stack） | 存放函数调用栈帧，包括局部变量、参数、返回地址等。 | 每次函数调用时自动分配，函数返回后自动释放。 | 增长方向：向低地址增长。<br>限制：默认大小有限（通常几MB），过深递归可能导致栈溢出。 |
...
| 堆（Heap） | 动态分配内存，用于malloc，calloc，new 等操作。 | 在运行过程中根据程序需求动态扩展。 | 增长方向: 向高地址增长。<br>管理方式: 通过brk和sbrk系统调用来调整堆顶。 |
