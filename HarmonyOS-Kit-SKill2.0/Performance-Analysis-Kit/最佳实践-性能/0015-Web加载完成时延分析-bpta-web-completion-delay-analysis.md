...
**图1** Web页面加载泳道图  ![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-001.png "点击放大")
...
**图2** Web加载完成时延分析流程  ![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-002.jpg "点击放大")
...
1. 使用Profiler抓取Trace，根据场景确定起止点。
  1. 确定Web加载完成时延Trace起点。点击切换到新的Web页面，以DispatchTouchEvent, type=1为起点。Web页面初始化加载以H:NWebImpl | CreateNWeb为起点。该Trace点位于应用主线程泳道内。该泳道负责应用主逻辑、接收多模信号、生成帧、分发子信号等。对应图3中的红色旗标处。
    **图3** Web页面初始化加载起点  ![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-003.png "点击放大")
  ...
  2. 确定Web加载完成时延Trace结束点。最后一个SkiaOutputSurfaceImplOnGpu::SwapBuffers为终点。该Trace点位于CompositorGpuTh泳道内。该泳道负责GPU光栅化处理，生成信号送图形子系统执行渲染。对应图4的紫色旗标处。
    **图4** Web页面初始化加载终点  ![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-004.png "点击放大")

2. 缩小Trace图，找到起点和终点，选中起点到终点范围内的Trace图，可查看当前Web页面的点击完成时延。如果该区域内加载完成时延超过900毫秒，使用下文介绍的DevTools进行性能分析。     ![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-005.png "点击放大")
...
1. 确认泳道的起止点，起点位于黄虚线处（0ms位置），终点为视口内容完全渲染上屏。
  **图5** 确定泳道起始点     ![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-006.png "点击放大")


  **图6** 确定泳道结束点     ![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-007.png "点击放大")

2. 常用泳道概览。DevTools提供了多个泳道为开发者提供性能分析数据，常用泳道如下图所示。![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-008.png "点击放大")
...
1. 某些网络请求会阻塞UI渲染，导致关键资源加载缓慢。业务侧需根据页面显示需求，分析并优化影响页面加载时延的关键请求，例如使用CDN、[预取POST](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-web-develop-optimization#section1742343332915)请求等。
  **图7** 网络请求阻塞UI渲染示意Trace图     ![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-009.png "点击放大")
...
2. 网络请求过多、服务器响应慢、无强依赖关系接口串行请求。此时的优化方案如下：
  ...
  **图8** 网络请求数量过多示意Trace图     ![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-010.png "点击放大")


  **图9** 服务器响应网络请求过慢示意Trace图  ![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-011.png "点击放大")

  **图10** 无强依赖接口串行请求示意Trace图  ![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-012.png)
...
![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-015.png "点击放大")
...
![](../assets/0015-Web加载完成时延分析-bpta-web-completion-delay-analysis-image-016.png "点击放大")
