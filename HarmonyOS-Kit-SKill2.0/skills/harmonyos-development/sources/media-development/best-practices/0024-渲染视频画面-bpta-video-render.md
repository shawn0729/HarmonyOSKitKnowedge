...
3. 启动解码器、解码输入子线程、解码输出子线程。     ![](../../_assets/images/bd58baf81015abfa41d45012.jpg "点击放大")

4. 通过OnNeedInputBuffer获取可用的AVBuffer后，在解码输入子线程中，将解封装器读取视频数据填充到AVBuffer中，并提交给解码器进行解码。     ![](../../_assets/images/2a54b6095e231df11d4a1fd1.jpg "点击放大")

5. 通过OnNeedOutputBuffer获取解码的视频数据后，在解码输出子线程中，将解码后的视频数据提交给输出Surface（即XComponent的NativeWindow）。     ![](../../_assets/images/19d30643cb762ecc4fd3fac1.jpg "点击放大")
...
3. 获取XComponent的NativeWindow对象，并根据XComponent的NativeWindow对象创建EGLSurface。     ![](../../_assets/images/d04a57184dcb573ff3171c9d.jpg "点击放大")
...
5. 启动解码器、解码输入子线程、解码输出子线程。     ![](../../_assets/images/bb55a6c3ece528139db783d5.jpg "点击放大")

6. 通过OnNeedInputBuffer获取可用的AVBuffer后，在解码输入子线程中，将解封装器读取视频数据填充到AVBuffer中，并提交给解码器进行解码。

7. 通过OnNeedOutputBuffer获取解码的视频数据后，在解码输出子线程中，将解码后的视频数据提交给输出Surface（当前Surface为NativeImage的NativeWindow对象）。     ![](../../_assets/images/dd1806ba459257f4e37accd6.jpg "点击放大")

8. 通过NativeImage，将视频图像缓存更新至OpenGL的纹理上。

9. 通过eglSwapBuffers交换前后缓冲区的内容，并将渲染结果显示在屏幕。     ![](../../_assets/images/37bea2a19dd340269b8753a5.jpg "点击放大")
...
3. 获取XComponent的NativeWindow对象，并根据XComponent的NativeWindow对象创建Vulkan的Surface用于显示。     ![](../../_assets/images/bd348b481ca3c2c19a88202d.jpg "点击放大")
...
9. Vulkan根据NativeBuffer创建对应的ImageView用于采样，同时，创建对应格式的采样器，通过采样器将YUV格式的图像转化成RGBA的图像，最后，通过渲染管道进行显示。     ![](../../_assets/images/c7a6d821a09a860650b9abbb.jpg "点击放大")
